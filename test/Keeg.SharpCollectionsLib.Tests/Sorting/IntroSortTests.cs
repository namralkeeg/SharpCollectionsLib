...
namespace Keeg.SharpCollectionsLib.Tests.Sorting
{
    public class IntroSortTests
    {
        ...
        internal static ISortAlgorithm<int> defaultSort = new IntroSort<int>();
        internal static ISortAlgorithm<int> customSort = new IntroSort<int>(new SortCustomComparer());
        ...
        {
            int nodes = 10000;
            var badList = Enumerable.Range(0, nodes).ToArray();

            defaultSort.Sort(badList, 0, nodes);
            for (int i = 0; i < nodes; i++)
            {
                Assert.Equal(i, badList[i]);
            }
        }
    }
}
